...
func (store *MessageStore) StoreMessage(id, chatJID, sender, content string, timestamp time.Time, isFromMe bool,
	mediaType, filename, url string, mediaKey, file_sha256, file_enc_sha256 []byte, fileLength uint64,
	isReply bool, quotedMessageID string, quotedSender string) error {
	...
	_, err := store.db.Exec(
		...
		(id, chat_jid, sender, content, timestamp, is_from_me, media_type, filename, url, media_key, file_sha256, file_enc_sha256, file_length, is_reply, quoted_message_id, quoted_sender) 
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		id, chatJID, sender, content, timestamp, isFromMe, mediaType, filename, url, mediaKey, file_sha256, file_enc_sha256, fileLength,
		isReply, quotedMessageID, quotedSender,
	)
	return err
}
...
// GroupMembersResponse represents the response for the get group members API
...
func handleMessage(client *whatsmeow.Client, messageStore *MessageStore, msg *events.Message, logger waLog.Logger) {
	...
	name := GetChatName(client, messageStore, msg.Info.Chat, chatJID, nil, senderJid, logger)
	...
	// --- Extract Reply Info (Corrected) ---
	var isReply bool = false
	var quotedMessageID string = ""
	var quotedSender string = ""

	// ContextInfo is usually within ExtendedTextMessage or other specific message types
	var ctxInfo *waProto.ContextInfo
	if extendedMsg := msg.Message.GetExtendedTextMessage(); extendedMsg != nil {
		ctxInfo = extendedMsg.ContextInfo
	} else if contactMsg := msg.Message.GetContactMessage(); contactMsg != nil {
		ctxInfo = contactMsg.ContextInfo
	} else if locationMsg := msg.Message.GetLocationMessage(); locationMsg != nil {
		ctxInfo = locationMsg.ContextInfo
	} // Add checks for other message types containing ContextInfo if needed...

	if ctxInfo != nil && ctxInfo.QuotedMessage != nil { // Check QuotedMessage exists
		isReply = true
		if ctxInfo.StanzaID != nil {
			quotedMessageID = *ctxInfo.StanzaID
		}
		if ctxInfo.Participant != nil {
			quotedSender = *ctxInfo.Participant
		}
	}
	// --- End Extract Reply Info ---
	...
	// Store message in database - Pass new reply info
	err = messageStore.StoreMessage(
		...
		fileEncSHA256,
		fileLength,
		isReply,         // Pass corrected value
		quotedMessageID, // Pass corrected value
		quotedSender,    // Pass corrected value
	)
	...
	if err != nil {
		...
		if mediaType != "" {
			fmt.Printf("[%s] %s %s: [%s: %s] %s\n", timestamp, direction, senderJid, mediaType, filename, content)
		} else if content != "" {
			fmt.Printf("[%s] %s %s: %s\n", timestamp, direction, senderJid, content)
		}
	}
}
...
func startRESTServer(client *whatsmeow.Client, messageStore *MessageStore, port int, logger waLog.Logger) {
	...
	router.HandleFunc("/api/download", func(w http.ResponseWriter, r *http.Request) {
		// Parse the request body
...
func writeJSONResponse(w http.ResponseWriter, data interface{}, statusCode ...int) {
	...
	if err != nil {
		// Log error, but can't write to response as headers are likely already sent
		// Use the logger passed into startRESTServer if available
		// logger.Errorf("Error encoding JSON response: %v", err)
		fmt.Printf("Error encoding JSON response: %v\n", err) // Fallback to fmt.Printf
	}
}

// Helper function to write JSON error responses
func writeJSONError(w http.ResponseWriter, message string, statusCode int) {
	writeJSONResponse(w, ErrorResponse{Success: false, Error: message}, statusCode)
}

// makeGroupMemberHandler creates a handler function that extracts the JID
// using gorilla/mux before calling the actual logic.
func makeGroupMemberHandler(client *whatsmeow.Client, logger waLog.Logger) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Log entry into the wrapper
		logger.Infof("Group member handler invoked for path: %s", r.URL.Path)

		// Method check is now handled by router.Methods(http.MethodGet)

		// Extract JID using mux.Vars
		vars := mux.Vars(r)
		jidStr, ok := vars["jid"]
		if !ok || jidStr == "" {
			logger.Warnf("Could not extract 'jid' variable from path: %s", r.URL.Path)
			writeJSONError(w, "Missing group JID in URL path", http.StatusBadRequest)
			return
		}

		// No need for manual path splitting/checking anymore
		logger.Infof("Extracted JID '%s' from path, calling handleGetGroupMembers", jidStr)
		handleGetGroupMembers(w, r, client, logger, jidStr)
	}
}

// handleGetGroupMembers retrieves and returns the members of a specific group.
func handleGetGroupMembers(w http.ResponseWriter, r *http.Request, client *whatsmeow.Client, logger waLog.Logger, jidStr string) {
	logger.Infof("Received request to get members for group: %s", jidStr)

	jID, err := types.ParseJID(jidStr)
	if err != nil {
		logger.Errorf("Failed to parse JID '%s': %v", jidStr, err)
		writeJSONError(w, fmt.Sprintf("Invalid group JID format: %s", jidStr), http.StatusBadRequest)
		return
	}

	if jID.Server != types.GroupServer {
		logger.Warnf("Attempted to get members for non-group JID: %s", jidStr)
	...
	if groupInfo == nil || groupInfo.Participants == nil {
		logger.Warnf("GetGroupInfo returned nil info or participants for %s", jidStr)
		writeJSONError(w, fmt.Sprintf("No participant data found for group %s", jidStr), http.StatusNotFound)
		return
	}
